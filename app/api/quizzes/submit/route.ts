...
export async function POST(req: NextRequest) {
  ...
  const persistentLimited = await persistentRateLimit({ scope: "quiz-submit", identifier: user.id, limit: 20, windowMs: 10 * 60_000 });
  ...
  const { data: session, error } = await admin
    .from("quiz_sessions")
    .select("id,user_id,level,subject,document_id,answer_key,total_questions,correct_answers,expires_at,submitted_at")
  ...
  const questions = z.array(storedQuestion).safeParse(session.answer_key);
