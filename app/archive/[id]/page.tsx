...
export default function ExamReaderPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data: examRow } = await supabase.from("exams").select("id,title,curriculum_reference").eq("id", params.id).eq("is_published", true).maybeSingle();
      if (!examRow) return setExam(null);
      const { data: files } = await supabase.from("exam_files").select("kind,storage_path").eq("exam_id", params.id);
    ...
    return (
      <NajahShell>
        <div className="rounded-3xl bg-white p-10 text-center">
          ...
          <h1 className="mt-4 text-2xl font-black text-emerald-950">هذا الامتحان غير متاح</h1>
          <Link href="/archive" className="mt-5 inline-flex items-center gap-1 rounded-xl border border-slate-200 px-4 py-2 font-bold">
            <ArrowRight className="size-4" />العودة إلى الأرشيف
          </Link>
        </div>
      </NajahShell>
    );
  ...
  return (
    <NajahShell>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <Link href="/archive" className="inline-flex items-center gap-1 text-sm font-bold text-emerald-800"><ArrowRight className="size-4" />العودة إلى الأرشيف</Link>
          <h1 className="mt-2 text-3xl font-black text-emerald-950">{exam.title}</h1>
          <p className="mt-2 text-sm text-slate-600">{exam.curriculum_reference || "لا يوجد مرجع تربوي مرفق بعد."}</p>
        </div>
        <span className="inline-flex items-center gap-2 rounded-xl bg-emerald-100 px-3 py-2 text-xs font-bold text-emerald-800">
          <ShieldCheck className="size-4" />وثيقة منشورة من الأرشيف
        </span>
      </div>
      <section className="mt-7 grid gap-5 lg:grid-cols-2">
        <PdfPanel title="موضوع الامتحان" examId={exam.id} storagePath={subject?.storage_path} />
        <PdfPanel title="عناصر الإجابة / التصحيح" examId={exam.id} storagePath={correction?.storage_path} />
      </section>
    </NajahShell>
  );
}

function PdfPanel({ title, examId, storagePath }: { title: string; examId: number; storagePath?: string }) {
  ...
  return (
    <article className="overflow-hidden rounded-3xl border border-emerald-950/10 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-emerald-950/10 px-5 py-4">
        ...
        {url && <a href={url} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-sm font-bold text-emerald-800"><Maximize2 className="size-4" />فتح</a>}
      </div>
      {url ? (
        ...
        <div className="grid min-h-80 place-items-center px-6 text-center">
          <div>
            ...
            <p className="mt-3 font-bold text-emerald-950">لم يُرفع هذا الملف بعد.</p>
            <p className="mt-1 text-sm leading-6 text-slate-600">لن يُعرض تصحيح بديل أو محتوى غير موثّق مكانه.</p>
          </div>
        </div>
      )}
    </article>
