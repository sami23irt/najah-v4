      ...
      <div className="grid gap-5 lg:grid-cols-[1.3fr_.9fr]">
          ...
          {joined ? (
            ...
            <div className="rounded-2xl bg-amber-50 p-4">
              ...
              <form className="mt-3 flex gap-2" onSubmit={e => { e.preventDefault(); join(accessCode); }}>
                <input value={accessCode} onChange={e => setAccessCode(e.target.value)} placeholder="رمز دعوة الغرفة الخاصة" type="password" className="flex-1 rounded-lg border border-slate-200 px-3 py-2" />
        ...
        <section className="overflow-hidden rounded-3xl border border-emerald-950/10 bg-white shadow-sm">
          ...
          <form onSubmit={sendMessage} className="flex gap-2 border-t border-emerald-950/10 p-4">
            <input value={body} onChange={e => setBody(e.target.value)} maxLength={1000} placeholder="اكتب رسالة…" className="flex-1 rounded-lg border border-slate-200 px-3 py-2" />
            <button type="submit" className="rounded-xl bg-emerald-900 px-4 py-2 text-white"><Send className="size-4" /></button>
          </form>
        </section>
      </div>
    </NajahShell>
