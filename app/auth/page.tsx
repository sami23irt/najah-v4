    ...
    <div className="moroccan-grid mx-auto grid max-w-5xl gap-8 rounded-[36px] p-5 md:grid-cols-[.9fr_1.1fr] md:p-8">
      ...
      <section className="najah-card p-6 sm:p-9">
        ...
        <form onSubmit={submit} className="mt-7 space-y-4">
          ...
          <label className="block space-y-2 text-sm font-bold text-slate-700">Mot de passe<div className="relative"><LockKeyhole aria-hidden="true" className="auth-field-icon absolute left-4 top-1/2 size-5 -translate-y-1/2 text-slate-400" /><input required minLength={6} type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} className="najah-input najah-input-leading-icon najah-input-trailing-icon" placeholder="6 caractères minimum" /><button type="button" onClick={() => setShowPassword(v => !v)} aria-pressed={showPassword} aria-label={showPassword ? "Masquer le mot de passe" : "Afficher le mot de passe"} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-lg p-1.5 text-slate-400">{showPassword ? <EyeOff className="size-5" /> : <Eye className="size-5" />}</button></div></label>
